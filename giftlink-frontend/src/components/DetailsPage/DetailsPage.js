...
function DetailsPage() {
	...
	useEffect( () => {
        //check if we have a valid authentication token in our session storage
        const authenticationToken = sessionStorage.getItem('auth-token');
        
        //if we don't have a valid authentiction token, redirect the user to the login page
        if (!authenticationToken) {
			// Task 1: Check for authentication and redirect
            navigate('/app/login');
        }

        //if we do have a valid authentication token...
